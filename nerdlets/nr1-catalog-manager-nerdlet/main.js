...
export default class GithubAbout extends React.PureComponent {
  ...
  renderTabs() {
    ...
    const getTab = function() {
      ...
      return visibleTab || 'repo';
      // return !isSetup ? 'setup' : 'repo';
    };
    ...
    return (
      <div className="container">
        ...
        <Tabs className="tabs" onChange={this.handleTabClick} value={getTab()}>
          ...
          <TabsItem value="repo" label="Repositories" disabled={!isSetup}>
            <GitHubQuery isSetup={isSetup} userToken={userToken} />
          </TabsItem>
          {/* <TabsItem value="deploy" label="Deploy" disabled={!isSetup}>
            <Deploy
              githubUrl={githubUrl}
